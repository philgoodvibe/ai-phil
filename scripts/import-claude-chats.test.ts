...
// =========================================================================
// 5b. stripSystemWrapperTags — slug should reflect Phillip's actual prompt,
// not local-command-caveat / command-name / system-reminder wrapper text.
// =========================================================================

Deno.test("stripSystemWrapperTags: removes local-command-caveat wrapper", () => {
  const input =
  ...
  assertEquals(stripSystemWrapperTags(input), "");
});

Deno.test("stripSystemWrapperTags: preserves real prompt after the wrapper", () => {
  const input =
    '<local-command-caveat>ignore me</local-command-caveat>\n<command-name>/clear</command-name>\n<command-message>clear</command-message>\n\nContinue the F.O.R.M. extractor fix.';
  assertEquals(
    stripSystemWrapperTags(input),
    "Continue the F.O.R.M. extractor fix.",
  );
});

Deno.test("stripSystemWrapperTags: strips system-reminder blocks", () => {
  const input =
...
Deno.test("firstUserText: falls through wrapper-only first turn to next real prompt", () => {
  const session: Session = {
    sessionId: "test",
    startedAt: "2026-04-21T00:00:00Z",
    endedAt: "2026-04-21T00:01:00Z",
    cwd: "/",
    toolUseCount: 0,
    turns: [
      {
      ...
      },
      {
        ...
        parts: [{ kind: "text", text: "fix the extractor" }],
      },
    ],
  };
  assertEquals(firstUserText(session), "fix the extractor");
});
