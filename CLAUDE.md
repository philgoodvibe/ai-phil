...
| Multi-file edge function deploys with `_shared/` imports (UPDATED 2026-04-19) | When deploying via Supabase MCP `deploy_edge_function`, the bundler prepends `source/` to every file `name`. Use `"_shared/salesVoice.ts"` (NOT `"../_shared/salesVoice.ts"`) so the uploaded files land at `source/_shared/*.ts` as siblings of `source/source/index.ts`. From the entrypoint the import `'../_shared/salesVoice.ts'` then resolves correctly. Verified: ghl-sales-agent v13 + ghl-member-agent v4 both deployed this way 2026-04-19. The earlier-documented `../_shared/*.ts` name was wrong. |
