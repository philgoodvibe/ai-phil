  ...
  try {
    const res = await fetch(
      `${GHL_API_BASE}/conversations/messages`,
      {
        ...
        body: JSON.stringify({ type: ghlType, contactId, message: replyText }),
      }
    );
    if (!res.ok) {
...
function salesSystemPrompt(
  ...
  return `You are the sales AI for AIAI Mastermind, speaking as Phillip Ngo.
...
function eventSystemPrompt(
  ...
  return `You are the events AI for AIAI Mastermind, speaking as Phillip Ngo.
...
function supportSystemPrompt(productsKb: string, firstName: string): string {
  return `You are a helpful assistant for AIAI Mastermind. Answer the question helpfully and briefly. Do not pitch the membership unless they directly ask about joining. Use "Hi ${firstName}", never "Hey". Be warm and direct.
    ...
    try {
      if (intent === 'sales') {
        ...
        replyText = await callClaude(
          modelUsed,
          200,
          supportSystemPrompt(productsKb, firstName || 'there'),
          messageBody
        );
      } else {
        // unknown — fallback copy, don't hit Claude
        modelUsed = 'fallback';
        replyText = `Hi ${firstName || 'there'}, thanks for the message. A human teammate will get back to you shortly.`;
      }
    } catch (err) {
